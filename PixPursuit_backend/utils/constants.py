...
MAIL_APP_PASSWORD = os.getenv(
    "MAIL_APP_PASSWORD"
)  # Application-specific password for mail server.
MAIL_FROM = os.getenv("MAIL_USERNAME")  # Email sender address.
MAIL_FROM_NAME = "PixPursuit"  # Name of the email sender.
CONFIRMATION_URL = (
    f"{os.getenv('NEXTAUTH_URL')}/verify-email?token="  # URL for email confirmation.
)
EMAIL_SUFFIX = "@pk.edu.pl"  # Suffix for email addresses.
EMAIL_SUBJECT = (
    "PixPursuit - Email Confirmation"  # Subject line for confirmation emails.
)
EMAIL_BODY_TEMPLATE = """
<html>
<head>
...
MODEL_FILE_PATH = os.path.join(
    get_generated_dir_path(), "tag_predictor_state.pth"
)  # File path for saving model state.
LEARNING_RATE = float(
...
ALLOWED_EXTENSIONS = {
    ...
    "jpg",
    "jpeg",
}  # Set of allowed file extensions for images.
PK_GALLERY_URL = (
    "http://www.galeria.pk.edu.pl/index.php?album="  # Base URL for the gallery.
)
...
DO_SPACE_ACCESS_KEY = os.getenv(
    "DO_SPACE_ACCESS_KEY"
)  # Access key for Digital Ocean space.
DO_SPACE_SECRET_KEY = os.getenv(
    "DO_SPACE_SECRET_KEY"
)  # Secret key for Digital Ocean space.
IMAGE_URL_PREFIX = (
    f"{DO_SPACE_ENDPOINT}/{BUCKET_NAME}/"  # Prefix for image URLs in the space.
)
...
UPDATE_AUTO_TAGS_SCHEDULE = "*/1"  # Schedule for updating auto tags.
CLUSTER_FACES_SCHEDULE = "*/1"  # Schedule for clustering faces.
BEAT_SCHEDULE_FILE_PATH = os.path.join(
    get_generated_dir_path(), "celerybeat-schedule"
)  # Path for Celery beat schedule file.
...
LOG_FILE_PATH = os.path.join(
    get_generated_dir_path(), "app.log"
)  # Path for the log file.
LOG_FORMAT = (
    "%(asctime)s - %(name)s - %(levelname)s - %(message)s"  # Format for log messages.
)
...
DISABLE_DEV_SHM_USAGE_ARG = (
    "--disable-dev-shm-usage"  # Disable dev shm usage argument for Selenium.
)
REMOTE_DEBUGGING_PORT_ARG = (
    "--remote-debugging-port=9222"  # Remote debugging port argument for Selenium.
)
...
DISABLE_EXTENSIONS_ARG = (
    "--disable-extensions"  # Disable extensions argument for Selenium.
)
...
SHAREPOINT_FETCH_HEADERS = {
    "Accept": "application/json;odata=verbose"
}  # Headers for sharepoint fetch request.
